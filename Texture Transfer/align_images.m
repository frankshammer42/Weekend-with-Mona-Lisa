...
tx = round((w1/2-cx1)*2);
if tx > 0,  im1 = padarray(im1, [0 tx],'replicate', 'pre');
else        im1 = padarray(im1, [0 -tx],'replicate', 'post');
end
ty = round((h1/2-cy1)*2);
if ty > 0,  im1 = padarray(im1, [ty 0],'replicate', 'pre');
else        im1 = padarray(im1, [-ty 0],'replicate', 'post');
end  
tx = round((w2/2-cx2)*2) ;
if tx > 0,  im2 = padarray(im2, [0 tx],'replicate', 'pre');
else        im2 = padarray(im2,[0 -tx], 'replicate', 'post');
end
ty = round((h2/2-cy2)*2);
if ty > 0,  im2 = padarray(im2,[ty 0], 'replicate', 'pre');
else        im2 = padarray(im2,[-ty 0], 'replicate', 'post');
end
